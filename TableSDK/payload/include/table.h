...
#ifndef STACK_TABLE_CAPACITY
#define STACK_TABLE_CAPACITY 100
#endif

#define ARENA_ALIGN(x) (((size_t)(x) + 7u) & ~(size_t)7u)
...
#define TABLE_STACK_DEFAULT_CAPACITY 32
    ...
    // Frees the Global Master Arena's backing memory and un-maps the singleton so a
    // later vGetTable(..., arena_bytes) can map a fresh one. Only call this after every
    // Table/TableStack/TableMap that used the arena has already been dropped - any slot
    // still pointing into the arena becomes dangling the instant this returns.
...
#define GET_UNION_MEMBER(elem_ptr, type)          (*(type*)((elem_ptr)->ptr))
    ...
    /* TableStack packs primitives directly into the pointer-sized `.ptr` cell of
     * each TableSlot_SA instead of allocating for them (that's the whole point
     * of the "zero-malloc" stack variant) - see push_raw_bytes() below. That
     * means a single cell can only ever hold sizeof(void*) bytes. On a 64-bit
     * build that's 8 bytes, so every primitive type (including TYPE_DOUBLE and
     * a 64-bit TYPE_LONG) fits. On a 32-bit build a cell is only 4 bytes, which
     * is still enough for TYPE_INT/TYPE_FLOAT/TYPE_SHORT/TYPE_CHAR/TYPE_LONG
     * (4 bytes on every mainstream 32-bit ABI) and every pointer-wrapper type
     * (they're always exactly sizeof(void*) by construction) - the one type
     * that doesn't fit is TYPE_DOUBLE, which is 8 bytes on every platform.
     *
     * This used to be a hard compile-time error (static_assert requiring a
     * 64-bit build), which meant the entire library refused to compile on a
     * 32-bit target even though only one type on one of its two container
     * variants is actually affected. That's now a purely informational build
     * message instead of a failure, and the affected calls already fail safely
     * at runtime: bPushDoubleStack()/PUSH_DOUBLE_STACK() on a build where a
     * cell is smaller than a double returns false and prints a clear error
     * (see push_raw_bytes()) rather than truncating the value or corrupting
     * memory. TABLE_STACK_SUPPORTS_DOUBLE below lets calling code check this
     * at compile time instead of discovering it from a failed push, and the
     * heap-based Table (bPushDouble/PUSH_DOUBLE) is unaffected either way - it
     * always allocates the exact size a type needs, on any architecture. */
#define TABLE_STACK_SUPPORTS_DOUBLE (sizeof(void*) >= sizeof(double))

     /* The warning above is deliberately informational-only, gated on compiler-
      * provided macros that ARE valid in #if (sizeof() itself is not - it needs
      * a real type, which doesn't exist yet at preprocessing time). GCC/Clang
      * expose __SIZEOF_POINTER__/__SIZEOF_DOUBLE__ for exactly this; MSVC has no
      * equivalent, so there we key off _WIN64 (MSVC only ever targets Windows,
      * so its absence means a 32-bit target). If neither is available the check
      * is simply skipped - TABLE_STACK_SUPPORTS_DOUBLE and the runtime rejection
      * in push_raw_bytes() are what actually keep this safe either way; this is
      * only a heads-up so a 32-bit build doesn't need to discover the limit
      * from a failed push. */
#if defined(__SIZEOF_POINTER__) && defined(__SIZEOF_DOUBLE__)
#if __SIZEOF_POINTER__ < __SIZEOF_DOUBLE__
    _Pragma("message(\"table.h: building for a 32-bit target - TableStack (the zero-malloc variant) cannot hold TYPE_DOUBLE, a cell is only sizeof(void*)=4 bytes. bPushDoubleStack() will return false at runtime; use the heap-based Table for doubles on this build. Guard with #if TABLE_STACK_SUPPORTS_DOUBLE if needed.\")")
#endif
#elif defined(_MSC_VER) && !defined(_WIN64)
#pragma message("table.h: building for a 32-bit target - TableStack (the zero-malloc variant) cannot hold TYPE_DOUBLE, a cell is only sizeof(void*)=4 bytes. bPushDoubleStack() will return false at runtime; use the heap-based Table for doubles on this build. Guard with #if TABLE_STACK_SUPPORTS_DOUBLE if needed.")
#endif
...
#define GET_MAP_VALUE(node_ptr, type) (*(type*)((node_ptr)->ptr))
    ...
    /* Sort / filter / transform / foreach layered on the core Table API below. */

    /* ------------------------------- FOREACH -------------------------------- */
    /* Inline iteration without writing a separate visitor function. `slot_var`
     * must already be declared as TableSlot_DA / TableSlot_SA in scope; `idx_var`
     * is declared by the macro itself.
     *
     *   TableSlot_DA slot;
     *   TABLE_FOREACH(&myTable, i, slot)
     *   {
     *       printf("%d: %s\n", i, sTypeNameTable(slot.type));
     *   }
     */
#define TABLE_FOREACH(tbl_ptr, idx_var, slot_var) \
    for (int idx_var = 0; \
         (idx_var) < (tbl_ptr)->count && (((slot_var) = (tbl_ptr)->slots[idx_var]), 1); \
         (idx_var)++)

#define STACK_FOREACH(stk_ptr, idx_var, slot_var) \
    for (int idx_var = 0; \
         (idx_var) < (stk_ptr)->count && (((slot_var) = (stk_ptr)->slots[idx_var]), 1); \
         (idx_var)++)

     /* ------------------------------- SORTING -------------------------------- */

    typedef int (*TableComparator)(const TableSlot_DA* a, const TableSlot_DA* b);
    typedef int (*TableStackComparator)(const TableSlot_SA* a, const TableSlot_SA* b);

    /* Reads a slot's payload as a double if its type is one of the six numeric
     * primitives (int/float/double/long/short/char); otherwise leaves *out
     * untouched and returns false. This is what lets sort/compare operate
     * across a heterogeneous container instead of a single fixed type. */
    extern bool bSlotAsNumber(TableSlot_DA slot, double* out);

    /* Built-in comparators for the common case: sort the numeric slots in a
     * heterogeneous Table by value, with non-numeric slots (structs, strings,
     * pointers, etc.) pushed to the end, ordered by type tag for determinism. */
    ...
    /* Stable O(n log n) merge sort in place over obj->slots[0..count).
     * Only the TableSlot_DA {ptr, type} pairs are reordered - the payloads
     * they point to never move, so this is safe regardless of ownership. */
    extern bool vSortTable(Table* obj, TableComparator cmp);
    extern bool vSortTableStack(TableStack* obj, TableStackComparator cmp);

    /* -------------------------------- FILTER --------------------------------- */

    typedef bool (*TablePredicate)(const TableSlot_DA* slot, void* ctx);

    /* Copies every slot from src for which pred returns true into dest (which
     * the caller must have already initialized with vGetTable). Ownership
     * semantics match bCloneTable exactly: owned primitive/pointer-wrapper
     * types are deep-copied (fresh allocation), raw/user-managed types are
     * copied by reference, and TYPE_STRUCT/TYPE_UNION slots are skipped with a
     * warning since their size isn't tracked anywhere to copy safely - clone
     * those manually with PUSH_STRUCT/PUSH_UNION if you need them filtered too.
     * Returns false only on allocation failure; a false predicate for every
     * slot is not an error and returns true with dest left empty. */
    extern bool bFilterTable(const Table* src, Table* dest, TablePredicate pred, void* ctx);

    /* ------------------------------- TRANSFORM -------------------------------- */

    /* Called once per source slot. Push whatever you want into `dest` (zero,
     * one, or several slots, of any type) using the normal PUSH_ macros or
     * bPush* functions - this is deliberately not a 1:1 value-swap, since a
     * heterogeneous container can't guess what "transformed" should mean for
     * every type. */
    typedef void (*TableTransform)(const TableSlot_DA* in, Table* dest, void* ctx);

    /* Runs fn once per slot in src, in order. dest must already be initialized. */
